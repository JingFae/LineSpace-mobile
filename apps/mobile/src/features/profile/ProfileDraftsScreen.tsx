  ...
  return (
    <AppScreen contentContainerStyle={styles.screen} padded={false} scroll={false} style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        {draftsQuery.isLoading ? <View style={styles.center}><ActivityIndicator color={colors.ink} /></View> : draftsQuery.isError ? <EmptyState body="Your drafts could not be loaded." title="Drafts unavailable" /> : draftsQuery.data?.items.length ? draftsQuery.data.items.map((draft) => <Pressable key={draft.id} onPress={() => router.push({ pathname: "/(tabs)/compose", params: { type: draft.mode === "relay" ? "thread" : "post", draftId: draft.id, session: `resume-${draft.id}` } } as Href)} style={({ pressed }) => [styles.card, pressed && styles.pressed]}><View style={styles.cardTop}><Text style={styles.cardStatus}>{draft.status === "ready" ? "READY TO PUBLISH" : "IN PROGRESS"}</Text><Text style={styles.date}>{formatDate(draft.updatedAt)}</Text></View><Text style={styles.cardTitle}>{draft.title.trim() || (draft.mode === "relay" ? "poem relay" : "untitled line")}</Text><Text numberOfLines={3} style={styles.cardBody}>{(draft.mode === "relay" ? (draft.relayFirstLine ?? draft.body) : draft.body).trim() || "A blank page is still a beginning."}</Text><Text style={styles.open}>continue writing →</Text></Pressable>) : <EmptyState body="Finish a poem later and it will stay here." title="No drafts yet" />}
      </ScrollView>
    </AppScreen>
  );
}
