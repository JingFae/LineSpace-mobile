...
`apps/api/src/database/deferred-migrations/` 是历史设计参考，不会被
`supabase db push` 自动执行，也不应复制到正式迁移目录。
...
├─ profile-repository.ts       # 用户资料、搜索、关注和最近联系人
├─ post-repository.ts          # Feed、Post、点赞、收藏和转发
├─ comment-repository.ts       # 评论及评论点赞/收藏
├─ thread-repository.ts        # Thread、Continuation、Thread 分享
├─ draft-repository.ts         # 草稿、协作操作、发布和 Storage 上传
├─ inbox-repository.ts         # 私聊、群聊和消息内容映射
